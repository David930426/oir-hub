...
export default function HomePage() {
  ...
  const stats = statsForYear("115");
  const calls = openBulletins().slice(0, 3);
  const latestPosts = publishedPosts().slice(0, 3);
  const openFunding = fundings.filter((f) => f.status === "open").slice(0, 3);
  const stories = publishedTestimonials().slice(0, 3);
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <CardDescription className="mt-1 max-w-xl">
                  ...
                  bring your transcript and your shortlist. {slots.length} slots
                  run every week.
                </CardDescription>
              </div>
              <Button asChild>
