...
import type { PostType } from "@/lib/mock";

const typeTabs: (PostType | "all")[] = ["all", "notice", "news", "guide", "page"];

export default function NewsPage() {
  const { t } = useLocale();
  const all = useMemo(() => publishedPosts(), []);
  ...
  const filtered = useMemo(
    () =>
      all.filter((p) => {
        ...
        if (tagId !== "all" && !tagsForPost(p.id).some((tg) => tg.id === tagId))
          return false;
        return true;
      }),
