...
export default function BulletinDetailPage({
  ...
  const schools = program
    ? schoolsForProgram(program.id).filter((s) => s.active)
    : [];
  const days = daysUntil(bulletin.deadlineAt);
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
      ...
      {bulletin.status === "open" && days >= 0 && days <= 14 && (
      ...
      {pdf ? (
        <div className="mb-10">
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            Version {pdf.version} · uploaded {pdf.createdAt}. Earlier versions
            are archived and no longer valid.
          </p>
        </div>
      ) : (
