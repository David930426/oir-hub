...
/** One active agreement as the public directory shows it. */
export type SiteSchool = {
  id: string;
  programId: string;
...
export function SchoolsView({
  ...
  // Built from the rows themselves: regions and countries are free text the
  // office maintains, so the filters should offer exactly what is on file.
  ...
  const filtered = useMemo(
    ...
    [partnerSchools, programId, region, country, myGpa, englishOnly, housingOnly, query]
  );
                  ...
                  <Card className="h-full transition-all group-hover:-translate-y-0.5 group-hover:border-primary/40 group-hover:shadow-md">
                    <CardHeader>
                      <div className="mb-1 flex flex-wrap items-center gap-2">
                        {s.programType && (
                          <EnumBadge value={s.programType} meta={programTypeMeta} />
                        )}
