...
export default function PartnerSchoolDetailPage({
  ...
  const school = getPartnerSchool(id);
  if (!school) notFound();

  const program = getProgram(school.programId);
  const brief = getMediaFile(school.briefFileId);
  const stories = testimonialsForSchool(school.id);
  const openCalls = program
    ? bulletinsForProgram(program.id).filter((b) => b.status === "open")
    : [];
