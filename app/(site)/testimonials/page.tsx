...
import { getPartnerSchool, publishedTestimonials } from "@/lib/mock";

export default function TestimonialsPage() {
  const { t } = useLocale();
  const all = useMemo(() => publishedTestimonials(), []);
  ...
  const filtered = useMemo(
    () =>
      all.filter((s) => {
        ...
        if (query) {
          const school = getPartnerSchool(s.partnerSchoolId);
          const haystack = `${s.displayName} ${s.deptYear} ${s.highlights.join(" ")} ${
            school ? `${school.name.zh} ${school.name.en ?? ""}` : ""
          }`.toLowerCase();
